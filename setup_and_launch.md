...
5. Build Slicer
```
ninja -j<N>    #N = # CPU cores. (nproc) is max, but likely will run out of RAM. wsl may require reconfiguring memory. Use 'make -j<N>' if not using ninja-build. If 32 GB or less, use N=2-4. If 64 GB (actually available, not just max) use 6-10. Be patient. This step may take multiple tries.
```
6. Run
```
cd ~/Slicer-SuperBuild-Debug/Slicer-build
...
```
bash /home/<user_name>/sm_manual/src/system_integration/run_needle_master.sh
```
